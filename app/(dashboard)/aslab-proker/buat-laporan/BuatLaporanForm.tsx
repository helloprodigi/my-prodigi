...
export function BuatLaporanForm({ allowedDivisionNames }: { allowedDivisionNames: string[] }) {
  ...
  useEffect(() => {
    fetch("/api/proker")
      .then((res) => res.json())
      .then((data) => {
        const pending = (data.programKerja ?? []).filter(
          (p: ProgramKerjaOption & { status: string }) =>
            p.status === "BELUM_SELESAI" && allowedDivisionNames.includes(p.divisi)
        );
